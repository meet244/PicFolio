...
export default function AutoAlbumDetailPage() {
  ...
  // Fetch auto album assets
  const fetchAutoAlbumAssets = useCallback(async () => {
    if (!currentUser || !albumName) return;
    ...
    try {
      ...
      formData.append("query", albumName.toLowerCase());
      formData.append("type", "auto albums");
      ...
      const photosByDateArray = data.map(([date, idsArray], index) => ({
        date,
        ...
        uniqueKey: `${date}-${index}`, // Add unique key combining date and index
      }));

      setPhotosByDate(photosByDateArray);
    } catch (error) {
      console.error("Error fetching auto album assets:", error);
      setError("Failed to load auto album. Please try again.");
    } finally {
      setLoading(false);
    }
  }, [currentUser, albumName]);
  ...
  // Fetch data separately to avoid infinite loops
  useEffect(() => {
    if (currentUser && albumName) {
      fetchAutoAlbumAssets();
    }
  }, [currentUser, albumName]);
  ...
  const handleDeletePhotos = async () => {
    ...
    try {
      ...
      // Refresh the auto album assets
      await fetchAutoAlbumAssets();
      setSelectedPhotos([]);
    } catch (error) {
          ...
          {!loading && !error && (
            <div className="space-y-8">
              {photosByDate.map((group) => (
                <DateSection key={group.uniqueKey} date={group.date}>
                  <PhotoGrid
                    photos={group.photos}
              ...
              {photosByDate.length === 0 && (
                <div className="text-center py-12">
                  <p className="text-gray-500 dark:text-gray-400">
                    No photos found in this auto album
                  </p>
                </div>
              )}
            </div>
